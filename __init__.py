...
bl_info = {
  ...
  "author": "Xembie, Seeeker, Joseph Eagar",
  "version": (0, 2, 0),
  "blender": (2, 80, 0),
...
def save_model(path,obj,scale,precision,basedir,shaderlist):
  objname = basedir + obj_prop(obj.quake3, "modelname",obj.name)
  ...
  if obj.quake3.animmodel == True:
    ...
    md3.name = obj.name
    startframe = str_to_int(obj_prop(obj.quake3,"startframe",0),0)
    md3.numFrames = str_to_int(obj_prop(obj.quake3,"numframes",1),1)
    if md3.numFrames < 1:
      md3.numFrames = 1
    ...
    for matindex in range(0,len(obj.material_slots)):
      ...
      if obj.material_slots[matindex].material.quake3.noimage == False:
        if image == None:
...
def quake_map(path,exportselected,scale,precision,texture,basedir,shaderfile,brushwidth,hiddentexture):
  ...
  for obj in expobjs:
    ...
    if obj.type == "MESH":
      ...
      if obj.quake3.convexbrush == True:
        ignorefaces = []
        while True:
          faces = []
          for face in mesh.polygons:
          ...
          for face in faces:
            if len(face.vertices) < 3:
              continue
            objtext = obj_prop(obj.quake3,"texture",texture)
            objdetailbrush = obj.quake3.detailbrush
            if len(mesh.materials) > face.material_index:
              objtext = obj_prop(mesh.materials[face.material_index].quake3,"texture",objtext)
              objdetailbrush = mesh.materials[face.material_index].quake3.detailbrush
        ...
        for face in mesh.polygons:
          if len(face.vertices) < 3:
            continue
          objtext = obj_prop(obj.quake3,"texture",texture)
          objhiddentext = obj_prop(obj.quake3,"hiddentexture",hiddentexture)
          objwidth = obj_prop(obj.quake3,"brushwidth",brushwidth)
          objdetailbrush = obj.quake3.detailbrush
          objhiddendetailbrush = obj.quake3.hiddendetailbrush
          if len(mesh.materials) > face.material_index:
            objtext = obj_prop(mesh.materials[face.material_index].quake3,"texture",objtext)
            objhiddentext = obj_prop(mesh.materials[face.material_index].quake3,"hiddentexture",objhiddentext)
            objwidth = obj_prop(mesh.materials[face.material_index].quake3,"brushwidth",objwidth)
            objdetailbrush = mesh.materials[face.material_index].quake3.detailbrush
            objhiddendetailbrush = mesh.materials[face.material_index].quake3.hiddendetailbrush
          br = []
          brushlist.append(br)
          localcenter = mathutils.Vector((0,0,0))
          for v in face.vertices:
      ...
      light = str_to_float(obj_prop(obj.quake3,"light",light),light)
      spawnflags = obj_prop(obj.quake3,"spawnflags","")
      if obj.quake3.junior == True:
...
class QuakeSceneSettings(bpy.types.PropertyGroup):
  exportselected  : BoolProperty(name = "Export selected objects",description = "Only export objects that are selected.",default = False)
  scale     : FloatProperty(name = "Scale",description = "Scale all objects from origin.",default = 64.0)
  precision   : IntProperty(name = "Precision",description = "Rounding of values to decimal place.",default = 5)
  basedir     : StringProperty(name = "Base directory",description = "Prefix for models and material textures",default = "models/")
  shaderfile    : StringProperty(name = "Shader file path",description = "Shader file path",default = "scripts/map.shader")
  texture     : StringProperty(name = "Texture",description = "Specific to some quake formats. This can be overrided per object",maxlen = 1024,default = "common/caulk")
  brushwidth    : FloatProperty(name = "Brush width",description = "Width of brushes. Scale is not applied.",default = 2.0)
  hiddentexture : StringProperty(name = "Hidden Texture",description = "Used on hidden brush faces.",maxlen = 1024,default = "common/invisible")

  skyboxdir : StringProperty(subtype = "DIR_PATH",name = "Save Directory",description = "Directory where skybox images should be saved.")
  skyboxname : StringProperty(name = "Base Name",description = "Base name or prefix for images.",default = "skybox")
  skyboxresx : IntProperty(name = "Image width",description = "Width of image in pixels.",default = 1024)
  skyboxresy : IntProperty(name = "Image height",description = "Height of image in pixels.",default = 1024)
  bsppath : StringProperty(subtype = "FILE_PATH",name = "BSP File",description = "BSP file to be modified.")
  lightgridscale : FloatProperty(name = "Scale",description = "Scale from Blender to Quake. Use same value used when exporting.",default = 64.0)
  lightgridlightscale : FloatProperty(name = "Light scale",description = "Multiplies the light color.",default = 1.0)
  extractentities : StringProperty(subtype = "FILE_PATH",name = "Output Text",description = "File path used to write entities text.")
  insertentities : StringProperty(subtype = "FILE_PATH",name = "Input Text",description = "File path used to read entities text.")
  extractshaders : StringProperty(subtype = "FILE_PATH",name = "Output Text",description = "File path used to write shader text.")
  insertshaders : StringProperty(subtype = "FILE_PATH",name = "Input Text",description = "File path used to read shader text.")
  convertbsp : StringProperty(subtype = "FILE_PATH",name = "Output BSP",description = "File path used to write converted BSP.")
  convertbspversion : IntProperty(name = "Version",description = "BSP version usually unique to each game.",default = 1,min = 0,max = 0x7FFFFFFF)
...
class ToolPanel(bpy.types.Panel):
  ...
  def draw(self,context):
    scene = context.scene
    box = self.layout.box()
    box.label(text="Create Skybox Images")
    box.prop(scene.quake3,"skyboxdir")
    box.prop(scene.quake3,"skyboxname")
    box.prop(scene.quake3,"skyboxresx")
    box.prop(scene.quake3,"skyboxresy")
    box.operator("quakemap.skybox")
    box = self.layout.box()
    box.label(text="IBSP/RBSP Operations")
    box.prop(scene.quake3,"bsppath")
    box2 = box.box()
    box2.label(text="Build Light-grid")
    box2.prop(scene.quake3,"lightgridscale")
    box2.prop(scene.quake3,"lightgridlightscale")
    box2.label(text="Save before continuing! This may take a very long time.")
    box2.label(text="May generate poor grid lights near walls.")
    box2.operator("quakemap.lightgrid")
    box2 = box.box()
    box2.label(text="Remove Directional Light-grid")
    box2.operator("quakemap.lightgridaverage")
    box2 = box.box()
    box2.label(text="Extract Entities")
    box2.prop(scene.quake3,"extractentities")
    box2.operator("quakemap.extractentities")
    box2 = box.box()
    box2.label(text="Insert Entities")
    box2.prop(scene.quake3,"insertentities")
    box2.operator("quakemap.insertentities")
    box2 = box.box()
    box2.label(text="Extract Shaders")
    box2.prop(scene.quake3,"extractshaders")
    box2.operator("quakemap.extractshaders")
    box2 = box.box()
    box2.label(text="Import Shaders")
    box2.prop(scene.quake3,"extractshaders")
    box2.operator("quakemap.insertshaders")
    box2 = box.box()
    box2.label(text="Convert BSP(IBSP<->RBSP)")
    box2.prop(scene.quake3,"convertbsp")
    box2.prop(scene.quake3,"convertbspversion")
...
class SkyboxButton(bpy.types.Operator):
  ...
  def execute(self,context):
    ...
    elif len(scene.quake3.skyboxdir) == 0 or len(scene.quake3.skyboxname) == 0:
      popup("Skybox","Missing skybox directory or name.","ERROR")
    elif scene.quake3.skyboxresx <= 0 or scene.quake3.skyboxresy <= 0:
      popup("Skybox","Image width and height must be at least 1.","ERROR")
    else:
      skybox(scene,camobj,scene.quake3.skyboxdir,scene.quake3.skyboxname,scene.quake3.skyboxresx,scene.quake3.skyboxresy)
    return{"FINISHED"}
def lightgrid_bake(origin,bounds,gridsize,scale):
  ...
  if bpy.context.scene.render.engine != "CYCLES":
    raise RuntimeError("set render engine to cycles")
...
class LightgridButton(bpy.types.Operator):
  ...
  def execute(self,context):
    scene = context.scene
    if len(scene.quake3.bsppath) == 0:
      popup("Light-grid","Missing BSP file path.","ERROR")
    elif bpy.context.scene.render.engine != "CYCLES":
      popup("Light-grid","Only works with cycles render","ERROR")
    else:
      lightgrid(scene.quake3.bsppath,scene.quake3.lightgridscale,scene.quake3.lightgridlightscale)
    return{"FINISHED"}
...
class LightgridAverageButton(bpy.types.Operator):
  ...
  def execute(self,context):
    ...
    if len(scene.quake3.bsppath) == 0:
      ...
      bsp = bsp_load(scene.quake3.bsppath)
      if bsp == None:
        popup("Remove Directional Light-grid","Could not load bsp " + scene.quake3.bsppath,"ERROR")
        return{"CANCELLED"}
      if bsp[0] == b"IBSP":
        newgrid = b""
        ...
        bsp[4][15] = newgrid
        bsp[4][17] = newarray
      if bsp_save(scene.quake3.bsppath,bsp[0],bsp[1],bsp[3],bsp[4]) == False:
        popup("Remove Directional Light-grid","Could not save bsp " + scene.quake3.bsppath,"ERROR")
        return{"CANCELLED"}
...
class ExtractEntitiesButton(bpy.types.Operator):
  ...
  def execute(self,context):
    ...
    elif len(scene.quake3.extractentities) == 0:
      popup("Extract Entities","Missing extract file path.","ERROR")
    else:
      bsp = bsp_load(scene.quake3.bsppath)
      if bsp == None:
        popup("Extract Entities","Could not load bsp " + scene.quake3.bsppath,"ERROR")
        return{"CANCELLED"}
      success = False
      with open(scene.quake3.extractentities,"wb") as f:
        entities = bsp[4][0]
        if entities[-1:] == b"\0":
          entities = entities[:-1]
        f.write(entities)
        ...
        success = True
      if success == False:
        popup("Extract Entities","Could not write to " + scene.quake3.extractentities,"ERROR")
        return{"CANCELLED"}
...
class InsertEntitiesButton(bpy.types.Operator):
  ...
  def execute(self,context):
    ...
    elif len(scene.quake3.insertentities) == 0:
      popup("Insert Entities","Missing insert file path.","ERROR")
    else:
      bsp = bsp_load(scene.quake3.bsppath)
      if bsp == None:
        popup("Insert Entities","Could not load bsp " + scene.quake3.bsppath,"ERROR")
        return{"CANCELLED"}
      success = False
      with open(scene.quake3.insertentities,"rb") as f:
        contents = f.read()
        contents.replace(b"\r\n",b"\n")
        ...
        success = True
      if success == False:
        popup("Insert Entities","Could not read from " + scene.quake3.insertentities,"ERROR")
        return{"CANCELLED"}
      if bsp_save(scene.quake3.bsppath,bsp[0],bsp[1],bsp[3],bsp[4]) == False:
        popup("Insert Entities","Could not save bsp " + scene.quake3.bsppath,"ERROR")
        return{"CANCELLED"}
...
class ExtractShadersButton(bpy.types.Operator):
  ...
  def execute(self,context):
    ...
    elif len(scene.quake3.extractshaders) == 0:
      popup("Extract Shaders","Missing extract file path.","ERROR")
    else:
      bsp = bsp_load(scene.quake3.bsppath)
      if bsp == None:
        popup("Extract Shaders","Could not load bsp " + scene.quake3.bsppath,"ERROR")
        return{"CANCELLED"}
      shaderarray = shader_parse(bsp[4][1])
      success = False
      with open(scene.quake3.extractshaders,"wb") as f:
        for shader in shaderarray:
          f.write(b"\"" + shader[0] + b"\",\"" + str(shader[1]).encode() + b"\",\"" + str(shader[2]).encode() + b"\"\n")
        f.close()
        success = True
      if success == False:
        popup("Extract Shaders","Could not write to " + scene.quake3.extractshaders,"ERROR")
        return{"CANCELLED"}
...
class InsertShadersButton(bpy.types.Operator):
  ...
  def execute(self,context):
    ...
    elif len(scene.quake3.insertshaders) == 0:
      popup("Insert Shaders","Missing insert file path.","ERROR")
    else:
      bsp = bsp_load(scene.quake3.bsppath)
      if bsp == None:
        popup("Insert Shaders","Could not load bsp " + scene.quake3.bsppath,"ERROR")
        return{"CANCELLED"}
      shaderarray = shader_parse(bsp[4][1])
      success = False
      with open(scene.quake3.insertshaders,"rb") as f:
        contents = f.read()
        ...
        if len(readarray) != len(shaderarray):
          popup("Insert Shaders","The number of shaders in " + scene.quake3.extractshaders + " does not match the BSP","ERROR")
          return{"CANCELLED"}
        bsp[4][1] = shader_string(readarray)
        success = True
      if success == False:
        popup("Insert Shaders","Could not read from " + scene.quake3.insertshaders,"ERROR")
        return{"CANCELLED"}
      if bsp_save(scene.quake3.bsppath,bsp[0],bsp[1],bsp[3],bsp[4]) == False:
        popup("Insert Shaders","Could not save bsp " + scene.quake3.bsppath,"ERROR")
        return{"CANCELLED"}
...
class ConvertBSPButton(bpy.types.Operator):
  ...
  def execute(self,context):
    ...
    elif len(scene.quake3.convertbsp) == 0:
      popup("Convert BSP","Missing output BSP file path.","ERROR")
    else:
      bsp = bsp_load(scene.quake3.bsppath)
      if bsp == None:
        popup("Convert BSP","Could not load bsp " + scene.quake3.bsppath,"ERROR")
        return{"CANCELLED"}
      format = bsp[0]
      version = bsp[1]
      try:
        version = int(scene.quake3.convertbspversion)
      except:
        pass
      if bsp[0] == b"IBSP":
        format = b"RBSP"
        ...
        bsp[4][15] = newgrid
        bsp[4][17] = b""
      if bsp_save(scene.quake3.convertbsp,format,version,bsp[3],bsp[4]) == False:
        popup("Convert BSP","Could not save bsp " + scene.quake3.convertbsp,"ERROR")
        return{"CANCELLED"}
      popup("Convert BSP","Completed conversion to " + format.decode(),"NONE")
      return{"FINISHED"}
    return{"CANCELLED"}

class QuakeObjectSettings(bpy.types.PropertyGroup):
  noexport : BoolProperty(name = "Do Not Export",description = "Object will not be exported.",default = False)
  shadows : BoolProperty(name = "Cast Shadows",description = "Model shader name will have \"_RMG_BSP\" appended instead of \"_BSP\"",default = False)
  autoclip : BoolProperty(name = "Auto-clip",description = "Generate a solid brush for model.",default = False)
  forcemeta : BoolProperty(name = "Force meta",description = "Allows model to have a lightmap.",default = False)
  modelbrush : BoolProperty(name = "Model Brush",description = "Allows model to have a brush set as well. Makes auto-clip redundant.",default = False)
  modelname : StringProperty(name = "Model Name",description = "For obj path, shader, and jpg texture paths.")
  animmodel : BoolProperty(name = "Animation",description = "Model will be exported as an MD3. Make sure mesh is triangulated.",default = False)
  fps : StringProperty(name = "FPS",description = "Frames per second. Default is 20.")
  startframe : StringProperty(name = "Start Frame",description = "Start frame number.")
  numframes : StringProperty(name = "Number of Frames",description = "Number of frames after start frame to export.")
  tagname : StringProperty(name = "MD3 Tag Name",description = "Set on objects parented to an Animation model object.",maxlen = 64)
  nomodelent : BoolProperty(name = "No Model Entity",description = "No misc_model or misc_anim_model entity is added for the model.",default = False)
  castshadows : BoolProperty(name = "Cast Shadows",description = "Model will cast shadows during light stage.",default = False)
  recvshadows : BoolProperty(name = "Receive Shadows",description = "Model will receive shadows during light stage.",default = False)
  convexbrush : BoolProperty(name = "Convex Brushes",description = "Each linked mesh of the object becomes a single brush.",default = False)
  detailbrush : BoolProperty(name = "Detail Brushes",description = "Brush faces will not be included in vis calculation. Can be overrided per material.",default = False)
  hiddendetailbrush : BoolProperty(name = "Hidden Detail Brushes",description = "Hidden Brush faces will not be included in vis calculation. Can be overrided per material.",default = False)
  texture : StringProperty(name = "Texture",description = "Texture for visible brush faces or models. Can be overrided per material.")
  hiddentexture : StringProperty(name = "Hidden Texture",description = "Texture for hidden(generated) brush faces. Can be overrided per material.")
  brushwidth : StringProperty(name = "Brush Width",description = "Thickness of each face. Default is 2.0.")
  classname : StringProperty(name = "Classname",description = "Entity or brush group classname.")
  targetname : StringProperty(name = "Target Name",description = "Entity or brush group targetname.")
  target : StringProperty(name = "Target",description = "Entity or brush group target.")
  origin : StringProperty(name = "Origin",description = "Overrides entity origin.")
  angles : StringProperty(name = "Angles",description = "Overrides entity angles.")
  junior : BoolProperty(name = "Junior",description = "Light source will only affect entities.",default = False)
  light : StringProperty(name = "Light",description = "Overrides calculated light intensity.")
  fade : StringProperty(name = "Fade",description = "Light attenuation for linear lights. Default is 1.0.")
  radius : StringProperty(name = "Radius",description = "Radius by distance. Default is 64.")
  sun : BoolProperty(name = "Sun",description = "Turns light into sun emitter. Default is off.",default = False)
  linear : BoolProperty(name = "Linear",description = "Light source will have a linear falloff(attenuation) with distance. Spot lights only have non-linear angled attenuation with fade of 1.",default = False)
  noangle : BoolProperty(name = "No Angle",description = "Light source will not have attenuation.",default = False)
  nogridlight : BoolProperty(name = "No grid light",description = "Light source will not have dynamic entity lighting.",default = False)

class QuakeMaterialSettings(bpy.types.PropertyGroup): 
  shader : StringProperty(name = "Shader",description = "Shader name for model surface material. Defaults to material name.")
  noimage : BoolProperty(name = "No Image",description = "Don't export image texture for material.")
  detailbrush : BoolProperty(name = "Detail Brush",description = "Brush faces will not be included in vis calculation. Overrides object's detail brush value.",default = False)
  hiddendetailbrush : BoolProperty(name = "Hidden Detail Brush",description = "Hidden Brush faces will not be included in vis calculation. Overrides object's hidden detail brush value.",default = False)
  texture : StringProperty(name = "Texture",description = "Texture for visible brush faces or models. Overrides object's texture value.")
  hiddentexture : StringProperty(name = "Hidden Texture",description = "Texture for hidden(generated) brush faces. Overrides object's hiddentexture value.")
  brushwidth : StringProperty(name = "Brush Width",description = "Thickness of each face. Default is 2.0")
...
class ObjectPanel(bpy.types.Panel):
  ...
  bl_context = "object"
  def draw(self,context):
    if context.object.quake3:
      self.layout.prop(context.object.quake3,"noexport")
      box = self.layout.box()
      box.label(text="Model Options(Object must have UV map)")
      box.prop(context.object.quake3,"modelbrush")
      box.prop(context.object.quake3,"modelname")
      box.prop(context.object.quake3,"nomodelent")
      box.prop(context.object.quake3,"castshadows")
      box.prop(context.object.quake3,"recvshadows")
      box2 = box.box()
      box2.label(text="Model Spawnflags(Not for Animation)")
      box2.prop(context.object.quake3,"shadows")
      box2.prop(context.object.quake3,"autoclip")
      box2.prop(context.object.quake3,"forcemeta")
      box2.operator("quakemap.spawnflags")
      box.prop(context.object.quake3,"animmodel")
      box2 = box.box()
      box2.label(text="Model Animation Options")
      box2.prop(context.object.quake3,"fps")
      box2.prop(context.object.quake3,"startframe")
      box2.prop(context.object.quake3,"numframes")
      box2.prop(context.object.quake3,"tagname")
      box = self.layout.box()
      box.label(text="Brush Options")
      box.prop(context.object.quake3,"convexbrush")
      box.prop(context.object.quake3,"detailbrush")
      box.prop(context.object.quake3,"hiddendetailbrush")
      box = box.box()
      box.label(text="Brush Overrides")
      box.prop(context.object.quake3,"texture")
      box.prop(context.object.quake3,"hiddentexture")
      box.prop(context.object.quake3,"brushwidth")
      box = self.layout.box()
      box.label(text="Entity Properties")
      box.prop(context.object.quake3,"classname")
      box.prop(context.object.quake3,"targetname")
      box.prop(context.object.quake3,"target")
      box.prop(context.object.quake3,"origin")
      box.prop(context.object.quake3,"angles")
      box = self.layout.box()
      box.label(text="Light Properties")
      box.prop(context.object.quake3,"junior")
      box.prop(context.object.quake3,"light")
      box.prop(context.object.quake3,"fade")
      box2 = box.box()
      box2.label(text="Spot Light Properties")
      box2.prop(context.object.quake3,"radius")
      box2.prop(context.object.quake3,"sun")
      box2 = box.box()
      box2.label(text="Light Spawnflags")
      box2.prop(context.object.quake3,"linear")
      box2.prop(context.object.quake3,"noangle")
      box2.prop(context.object.quake3,"nogridlight")
...
class Spawnflags(bpy.types.Operator):
  ...
  def execute(self,context):
    val = 0
    if context.object.quake3.shadows:
      val |= 1
    if context.object.quake3.autoclip:
      val |= 2
    if context.object.quake3.forcemeta:
      val |= 4
...
class LightSpawnflags(bpy.types.Operator):
  ...
  def execute(self,context):
    val = 0
    if context.object.quake3.linear:
      val |= 1
    if context.object.quake3.noangle:
      val |= 2
    if context.object.quake3.nogridlight:
      val |= 16
    context.object["spawnflags"] = val
    return{'FINISHED'}
...
class MaterialPanel(bpy.types.Panel):
  ...
  def draw(self,context):
    if context.material:
      box = self.layout.box()
      box.label(text="Model Options")
      box.prop(context.material.quake3,"shader")
      box.prop(context.material.quake3,"noimage")
      box = self.layout.box()
      box.label(text="Brush Options")
      box.prop(context.material.quake3,"detailbrush")
      box.prop(context.material.quake3,"hiddendetailbrush")
      box = box.box()
      box.label(text="Brush Overrides")
      box.prop(context.material.quake3,"texture")
      box.prop(context.material.quake3,"hiddentexture")
      box.prop(context.material.quake3,"brushwidth")

classes = [
  QuakeSceneSettings,
  QuakeMaterialSettings,
  QuakeObjectSettings,
  ExportBrushMap,
  ToolPanel,
  SkyboxButton,
  LightgridButton,
  LightgridAverageButton,
  ExtractEntitiesButton,
  InsertEntitiesButton,
...
def register():
  ...
  bpy.types.Scene.quake3 = bpy.props.PointerProperty(type=QuakeSceneSettings);
  bpy.types.Object.quake3 = bpy.props.PointerProperty(type=QuakeObjectSettings);
  bpy.types.Material.quake3 = bpy.props.PointerProperty(type=QuakeMaterialSettings);
